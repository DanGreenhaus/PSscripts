...
ForEach($server in $servers) {
 #If only running on a single endpoint, add the following line: $server= <hostname>
  ...
  invoke-command -ComputerName $server -ScriptBlock {
    #if you want to run this on a local machine, just run this block
    $InstalledSoftware = Get-ChildItem "HKLM:\Software\Microsoft\Windows\CurrentVersion\Uninstall"
    foreach($obj in $InstalledSoftware){
        if ($obj.GetValue('DisplayName') -like "*Chrome*") {write-host $obj.GetValue('DisplayName') -NoNewline;`
    ...
    foreach($obj in $HKU){
      if ($obj.GetValue('DisplayName') -like "*Chrome*") {write-host $obj.GetValue('DisplayName') -NoNewline; `
      write-host " - " -NoNewline; write-host $obj.GetValue('DisplayVersion') -nonewline; write-host " - "  -NoNewline;Write-Host $obj.GetValue('UninstallString'); `
      write-host $obj.GetValue('DisplayVersion') -nonewline; write-host " - "  -NoNewline;Write-Host $obj.GetValue('UninstallString') }
    }
