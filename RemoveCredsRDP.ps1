...
(Get-Content -Raw $path\$file) -replace "(?sm)<credentialsProfile.*?(?=</credentialsProfile>)", '' |  ForEach-Object {$_ -replace " </credentialsProfile>",""} |  ForEach-Object {$_ -replace "</credentialsProfiles>",""} |Set-Content $path\Credential-less_$file

(Get-Content -Raw $path\Credential-less_$file) -replace "(?sm)<logonCredentials.*?(?=</logonCredentials>)", '' |  ForEach-Object {$_ -replace "</logonCredentials>",""} | Set-Content $path\Credential-less_$file
